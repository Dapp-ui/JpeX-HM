...
<div class=" bg-white rounded-2xl fixed object-center p-10 flex flex-col gap-3">
	...
	<div class=" flex-1 flex flex-col justify-between border border-[#E2D8FF] rounded-xl p-6 gap-4 ">
		...
		{#if $showProcessingForDeposit === false}
			<div
				...
				on:click={async () => {
					let tx = await depositInOptionBuyersVault($depositAmount, $selectedStrikePrice);
		...
		{:else}
			<button class=" bg-[#3E2681] w-full p-2 flex justify-center rounded-xl text-white cursor-pointer">
