...
<div
	class=" bg-white w-[550px] rounded-2xl fixed object-center p-10 flex flex-col gap-3 font-poppins"
>
	...
	<div class=" flex-1 flex flex-col justify-between border border-[#E2D8FF] rounded-xl p-6 gap-4 ">
		<!-- Card -->
		<div class=" bg-[#6129FF] flex flex-col p-6 items-center gap-3 rounded-xl">
			...
			<p class=" text-[#CAB8FF] font-medium">
				You’ve successfully deposited <span class=" font-bold text-white">{$selectedNftCollection + " #" + $selectedNft}</span>
			</p>
			...
			<p class=" text-[#FFFFFF] font-bold">
				{getOptionName($currentEpoch.toString(), $selectedStrikePrice, $epochExpiry, $selectedNftCollection)}
			</p>
		...
		<div class=" flex flex-row justify-between p-5 bg-gray-200 rounded-lg">
			...
			<div class="flex flex-col">
				...
				<div>
					<p class=" text-right font-medium text-[#C64DFF] py-2">0msx836930...87r398</p>
				</div>
			</div>
		</div>
		...
		<div class=" flex justify-center gap-2">
			...
			<a href={$rinkeByBaseUrl + $recentTxHash} target="_blank" class=" cursor-pointer">
				<svg
					width="24"
					height="24"
					viewBox="0 0 24 24"
