...
{#if $showProcessingForApprove === false}
	<button
		...
		on:click={async () => {
			const weth = getBySymbol('weth');

			// Approve for buyers vault
			let tx = await approveERC20(weth.address, contractAddress.NftOptionBuyersVault);
			showProcessingForApprove.set(true);
			let res = await tx.wait();

			// Approve for sellers vault
			tx = await approveERC20(weth.address, contractAddress.NftOptionSellersVault);
			showProcessingForApprove.set(true);
			res = await tx.wait();
