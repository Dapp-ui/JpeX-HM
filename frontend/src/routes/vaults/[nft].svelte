<script lang="ts">
	...
	import StrikeDropdown from '$lib/product/StrikeDropdown.svelte';
	...
	afterUpdate(async () => {
		if ($address !== '') {
			...
			if ($nftsLocked == undefined || $nftsLocked == []) {
				...
				const epochShorts = await getActiveShortsForEpoch($address, $currentEpoch.toString());
				const nftsLocked_ = [];
...
<div class=" flex flex-col bg-gray-50">
	<Header />
	<div
		class=" flex flex-row flex-1 p-1 overflow-y-auto  max-h-screen font-poppins "
		in:fly={{
			duration: 300,
			x: 0,
			y: 40,
	...
	>
		...
		<div class=" w-3/5  p-4">
			<div class=" h-full border rounded-xl p-1 bg-gray-100">
				<div class=" relative h-full border rounded-xl bg-white flex flex-col p-8 gap-3">
					<!-- Floor price on top of pfp -->
					<div class=" absolute w-max p-[2px] mt-4 ml-3 rounded-xl flex flex-col">
					...
					<div class="flex flex-row items-center">
						<!-- left pfp -->
						<img class=" h-[350px] p-2 rounded-2xl" src={nftCollectionImgList[0]} alt="" />
						<div class="p-2 ml-3">
							<!-- Card copy -->
							<div class="p-2 grid grid-cols-1 gap-2">
								...
								<div class="grid grid-cols-5 gap-3">
									<img class=" rounded-md" src={nftCollectionImgList[1]} alt="" />
									<img class=" rounded-md" src={nftCollectionImgList[2]} alt="" />
									<img class=" rounded-md" src={nftCollectionImgList[3]} alt="" />
									<img class=" rounded-md" src={nftCollectionImgList[0]} alt="" />
									<div class=" relative rounded-md">
										<img class=" absolute rounded-md" src={nftCollectionImgList[3]} alt="" />
										<div
											class=" absolute  text-white font-base text-sm font-poppins bg-black h-full w-full opacity-80 rounded-md p-2"
										>
											<p>10k</p>
											<p>items</p>
										</div>
								...
								<p class=" font-poppins font-[400] text-[12px] py-2">
									{$baycDescription}
								</p>
								<!-- Options : oracle price and expiry -->

								<div class=" flex flex-col  p-2 border border-[#DCDCDC] rounded-lg">
									<div class="flex flex-row justify-between">
										<div>
											<p class=" text-[#6F6F6F]  text-left text-sm font-[400]">Epoch</p>
										</div>
										<div>
									...
									<div class="flex flex-row justify-between">
										<div>
											<p class=" text-[#6F6F6F] text-left text-sm font-[400]">Expiry</p>
										</div>
										<div>
											<p class=" text-left text-sm ">{formatExpiry($epochExpiry)}</p>
										</div>
									</div>
								</div>
								...
								<div class=" flex flex-row justify-between ">
									<div class="flex flex-col gap-2">
										<div>
											<p class=" text-[#000000] text-left font-light text-[12px] ">
												Current Deposits
											</p>
										</div>
										<div>
											<p class=" text-left text-sm font-medium text-[#D06CFF]">25 WETH</p>
										</div>
									</div>
									<div class="flex flex-col gap-2">
										<div>
											<p class=" text-[#000000] text-right font-light text-[12px] ">Max Capacity</p>
										</div>
										<div>
								...
								<div class="w-full bg-gray-200 h-1.5 rounded-xl">
									<div class="bg-[#D06CFF] h-1.5 rounded-xl" style="width: 25%" />
								</div>
							</div>
							<!-- Card copy ended-->
						</div>
					...
					<!-- vault strategy, recent transaction and vault info -->
					<div
						class=" flex flex-row font-poppins items-center mt-3 text-[#54587B] font-[500] text-[14px] justify-start"
					>
						<div
							class={`rounded-2xl  px-4 py-2 cursor-pointer ${
								$selectedSectionInVaults === 0 ? ' bg-[#363636] text-white ' : ' text-[#54587B] '
							}`}
							on:click={() => selectedSectionInVaults.set(0)}
						>
							Vault Strategy
						</div>
						<div
							class={`ml-3 rounded-2xl  px-4 py-2 cursor-pointer ${
								$selectedSectionInVaults === 1 ? ' bg-[#363636] text-white ' : ' text-[#54587B] '
							}`}
							on:click={() => selectedSectionInVaults.set(1)}
						>
							Recent Transactions
						</div>
						<div
							class={`ml-3 rounded-2xl  px-4 py-2 cursor-pointer ${
								$selectedSectionInVaults === 2 ? ' bg-[#363636] text-white ' : ' text-[#54587B] '
							}`}
							on:click={() => alert('Soon')}
						>
							Vault Info
					...
					{#if $selectedSectionInVaults === 1}
						<div
							class=" bg-[#F6F6F6] rounded-lg grid grid-cols-5 font-poppins items-center mt-1 text-[#54587B] font-[500] text-[14px] justify-start py-2 px-4"
						>
							<p>ACTION</p>
							<p class=" ">NFT VAULT</p>
							<p class=" ">QUANTITY</p>
								...
								{#each $longPositions as position}
									...
									>
										<div class=" flex py-2 px-3 items-center ">
											<img class=" rounded-md  h-[40px]" src="/bayc/bayc-8.png" alt="" />
										...
										<div class=" grid grid-cols-1">
											<p>{position.option}</p>
											<p class=" font-light text-[10px]">Strike Price {position.strike} WETH</p>
										</div>
										<p class="ml-4">{position.quantity}</p>
										<div class="">
											<p class=" text-[#24A065]">+{position.premium} WETH</p>
										</div>
										<a href={position.txUrl} target="_blank" class=" p-2 flex justify-center">
											<svg
												width="24"
												height="24"
												viewBox="0 0 24 24"
								...
								{#each $shortPositions as position}
									...
									>
										<div class=" flex p-2 items-center">
											<img class=" rounded-md  h-[40px]" src={position.pfpUrl} alt="" />
											<div class=" flex items-center ml-2 text-[10px] gap-1">
												<p
		...
		<!-- right section -->
		<div class=" w-2/5 p-4 max-w-full ">
			<div class=" border rounded-xl p-1 bg-gray-100 max-h-full overflow-y-auto items-center">
				<div class=" h-full w-full border rounded-xl bg-white flex flex-col p-8">
					<!-- speculator writer switch -->
					<div class=" bg-[#000000] rounded-xl flex flex-col justify-between p-4  text-white">
						<div class=" flex flex-row justify-center items-center">
							...
							<div
								class={`h-6 w-11  flex rounded-2xl mr-2 items-center  cursor-pointer ${
									$optionViewer == 'buyer' ? ' justify-start bg-white ' : ' justify-end bg-white'
								}`}
						...
						<div class="flex justify-center font-poppins font-thin text-[14px] mt-4">
							Switch between buyers and sellers
						</div>
					</div>

					<!-- deposit withdraw -->
					<div class=" flex flex-row justify-start  items-center font-poppins mt-4 cursor-pointer">
						<div
					...
					{:else}
						...
						<div class="flex justify-center p-1 rounded-lg mt-3">
							<p class="font-poppins text-[11px] text-[#54587B] font-light">
								Deposits for Doodles Vault will open on 25 February 2022. Learn more
							</p>
						</div>
						<!-- divider -->
						<!-- Divider -->
						...
						<p class="  mt-3 font-poppins font-bold text-[20px]">NFTs locked in this vault</p>
							...
							<div class="flex flex-row font-poppins text-[12px] w-max">
								{#if $nftsLocked != undefined}
									{#each $nftsLocked as nft}
										...
										>
											<img class=" rounded-xl  h-[100px]" src={nft.pfpUrl} alt="" />

											<div class="flex flex-col gap-2 justify-center p-2 text-sm">
												<p class=" text-[#58595E] font-medium">
													Strike Price: <span class=" text-[#D06CFF] font-semibold"
														>{nft.strike} WETH</span
													>
												</p>
												<p class=" text-[#58595E] font-medium">
													Expiry: <span class=" text-black">{formatExpiry(nft.expiry)}</span>
												</p>
												<div class=" rounded-xl bg-white w-max border border-[#E7E7E7]">
													<p class=" text-[#58595E]  p-2 font-medium">
														Owner: <span class=" font-normal">{getShortAddress(nft.owner)}</span>
													</p>
												</div>
											</div>
										</div>
									{/each}
								{:else}
									<p class="flex flex-row overflow-x-auto font-poppins text-[12px] cursor-pointer">
										Loading ...
									</p>
								{/if}
							</div>
