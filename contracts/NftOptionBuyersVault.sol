...
contract NftOptionBuyersVault is Authorizable {
    ...
    /// @dev user participated in particular strike in epoch
    // mapping (uint256 => mapping (uint256 => address[])) public usersInEpochStrike;

    // mapping (uint256 => mapping (bytes32 => uint256)) public userStrikeEarningsInEpoch;

    // mapping (uint256 => mapping (bytes32=>uint256)) userStrikeShareInEpoch;
