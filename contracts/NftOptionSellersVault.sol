...
contract NftOptionSellersVault is IERC721Receiver, Authorizable{
    ...
    /// @notice start a new batch, renew all deposits
    ...
    function setStrikes(uint256[] memory strikes) public {
       // Check deposits haven't begun yet before setting strikes
    ...
    /// @dev mapping (epoch => (abi.encodePacked(user, strike) => tokenId array deposits))
    ...
    // mapping (epoch => (userstrikeEncoded => tokenids[]) )
    // one way to withdraw all nfts if strike cleared
    ...
    */
    ...
    function depositNftAndMintOption(uint256 tokenId, uint256 strike_) external{
        ...
        // Mint Option and send it to writer
        // IERC20(epochStrikeTokens[currentEpoch][strike_]).safeTransfer(
        //     address(this),
        //     BASE_UNIT
        // );

        /// @dev transfer premium worth that option to the minter
    ...
    function isNftClaimable(address user, uint256 tokenId) external view returns(uint256) {
    ...
    function calculatePNL(uint256 strike_) public view returns (uint256) {
    ...
    // Liquidation to be done manually in the beginning
